...
try:
  devid = os.environ['BLINKA_FT232H']
   ...
   print("Need to set 'BLINKA_FT232H=1'")
   print("Use export BLINKA_FT232H=1")
   print("Exiting early")
   sys.exit()
else:
  if (devid != '1'):
    print("Need to set 'BLINKA_FT232H=1', value found=%s"%(devid))
    print("Use export BLINKA_FT232H=1")
    print("Exiting early")
    sys.exit()
  # endif unexpected id
...
def LMX2820InitRegs(LMX):
    ...
    LMX.newreg[0] = 0x6070                               #0b0100000001110000     #Reset register PFD = 100 MHz Setting Fcal HPFD and LPFD ADJ.
    # !!!
    ...
    LMX.newreg[15] = 0x2001                      #0b0010000000000001    #Setting negative polairty for PFD charge pump to nomral operation
    LMX.newreg[16] = 0x171C                       #0b0001011100011100    #Setting charge pump current   Reset: 0b0010011100011100
    LMX.newreg[17] = 0x15C0                       #0b0001010111000000    #Setting lock det type to continious  Reset: 1010001000000
    ...
    LMX.newreg[32] = 0x1001                       #0b0001000000000001    #Setting Both Output vhannel dividers to 0 (Div by 2)
    LMX.newreg[33] = 0x0000                  #Reserved
    LMX.newreg[34] = 0x0010                            #0b0000000000010000    #Disabling External VCO and LoopBack
    LMX.newreg[35] = 0x3100                      #0b0011000100000000    #Enable Mash Reset and Set MASH order

    # !!! Reg 36 is 0x001E for Default, 0x012C for 10MHz !!!  
    LMX.newreg[36] = 0x012C                  #Setting INT value 
    # !!!

    LMX.newreg[37] = 0x0500                 #Setting PFD_DEL, only active whe PFD_DLY_MANUAL is 1, which it's NOT
    ...
    LMX.newreg[66] = 0x003F                  #Setting 2 of 4 JESD registers, sum of 2 registers must be equal or greater than 63
    ...
    LMX.newreg[70] = 0x000E                  #Setting Registers as double buffered, Changes to regs will only be enabled after a write to R0
...
def LMX2820calcIntNumDen(LMX,newFreq):
    #newParams = fractions()
    Fpfd = int(LMX.RefFREQ * ((1 + LMX.RefDoubler) * LMX.RefMultipler) // (LMX.PreRDiv * LMX.PostRDiv))  #Calculating Phase Frequency detector frequency
...
def LMX2820setREGSforNewFreq(LMX,newFreq):
    ...
    if newFreq < 44.140265e6 or newFreq > 22.6e9:                          #Making sure the requested freuqency falls withing accetable range
        ...
        realFreq = int(newFreq*LMX.DividerLUT[outputDividerIndex])              #Define the real frequency as the requested frequency * the necessary divider
...
if __name__ == '__main__':
  ...
  LMX = LMX2820(10e6,   # RefFREQ                  # initiation of class __init__
